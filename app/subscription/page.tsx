...
const Subscription = () => {
  ...
  const onSubscribe = async () => {
    try {
      ...
      console.error("subscription POST에서 API 오류 발생", error);
      if (axios.isAxiosError(error)) {
        return toast("오류 발생", {
          description: error.response?.data,
        });
      } else {
        console.error("subscription POST에서 서버 오류 발생", error);
        return toast("서버 오류 발생", {
          description: "서버에서 오류가 발생하였으니 잠시 후 다시 시도해주세요.",
        });
      }
    }
  };
  ...
  return (
    <div className="px-4 flex flex-col space-y-6 lg:px-0">
      ...
      <div className="lg:flex lg:justify-between space-x-4">
        <div className="bg-yellow-100 rounded-md flex flex-col space-y-2 p-4 border-2 border-transparent hover:border-yellow-200 shadow-md transition-colors lg:w-[70%]">
